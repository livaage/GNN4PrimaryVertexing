...
# from torch_geometric.data import Data, DataLoader #Deprecated
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PyG Interaction Network Implementation"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--construction",
        type=str,
        default="heptrkx_classic",
        help="graph construction method",
    )
    parser.add_argument(
        "--sample", type=int, default=1, help="TrackML train_{} sample to train on"
    )
    parser.add_argument(
    ...
    indir = "built_graphs/rebalanced_0_15/"
    ...
    partition = {
        "train": graph_files[IDs[: int(n_graphs * 0.8)]],
        "test": graph_files[IDs[int(n_graphs * 0.8) : int(n_graphs * 0.9)]],
        "val": graph_files[IDs[int(n_graphs * 0.9) : n_graphs]],
    }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.save_model:
            torch.save(model.state_dict(), "trained_models/epoch{}.pt".format(epoch))
